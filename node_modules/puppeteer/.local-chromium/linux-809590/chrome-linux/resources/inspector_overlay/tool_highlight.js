...
const width=canvasWidth/pageFactor;const height=canvasHeight/pageFactor;const gridSubStep=5;const gridStep=50;{context.save();context.fillStyle=gridBackgroundColor;if(rulerAtBottom){context.fillRect(0,zoom(height)-15,zoom(width),zoom(height));}
...
context.restore();}
context.restore();}
function doReset$1(){document.getElementById('tooltip-container').removeChildren();document.getElementById('grid-label-container').removeChildren();window._gridPainted=false;}
...
return null;}
function _createElementDescription(elementInfo,colorFormat){const elementInfoElement=createElement('div','element-info');const elementInfoHeaderElement=elementInfoElement.createChild('div','element-info-header');const layoutType=_getElementLayoutType(elementInfo);if(layoutType){elementInfoHeaderElement.createChild('div',`element-layout-type ${layoutType}`);}
...
else{arrowX=Number.constrain(containerMinX,xFromLeftBound,xFromRightBound);}}
...
if(highlight.paths.length){if(highlight.showExtensionLines){drawRulers(context,bounds,rulerAtRight,rulerAtBottom);}
if(highlight.elementInfo){_drawElementTitle(highlight.elementInfo,highlight.colorFormat,bounds);}}
if(highlight.gridInfo){for(const grid of highlight.gridInfo){drawLayoutGridHighlight(grid,context);}}
context.restore();return{bounds:bounds};}
window.setPlatform=function(platform){adoptStyleSheet(style);const gridStyleSheet=new CSSStyleSheet();gridStyleSheet.replaceSync(gridStyle);adoptStyleSheet(gridStyleSheet);document.body.classList.add('fill');const canvas=document.createElement('canvas');canvas.id='canvas';canvas.classList.add('fill');document.body.append(canvas);const tooltip=document.createElement('div');tooltip.id='tooltip-container';document.body.append(tooltip);const gridLabels=document.createElement('div');gridLabels.id='grid-label-container';document.body.append(gridLabels);setPlatform(platform);};window.reset=function(data){reset(data);doReset$1();doReset();};window.drawHighlight=drawHighlight;window.drawGridHighlight=drawGridHighlight;window.dispatch=dispatch;}());