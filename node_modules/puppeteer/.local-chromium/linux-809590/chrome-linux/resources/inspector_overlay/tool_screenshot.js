...
const style=new CSSStyleSheet();style.replaceSync("body {\n  cursor: crosshair;\n}\n\n#zone {\n  background-color: #0003;\n  border: 1px solid #fffd;\n  display: none;\n  position: absolute;\n}\n");let anchor=null;let position=null;function currentRect(){return{x:Math.min(anchor.x,position.x),y:Math.min(anchor.y,position.y),width:Math.abs(anchor.x-position.x),height:Math.abs(anchor.y-position.y)};}
...
cancel();updateZone();event.stopPropagation();event.preventDefault();},true);document.documentElement.addEventListener('mousemove',event=>{if(anchor&&event.buttons===1){position={x:event.pageX,y:event.pageY};}
else{anchor=null;}
...
window.setPlatform=function(platform){adoptStyleSheet(style);document.body.onload=loaded;const zone=document.createElement('div');zone.id='zone';document.body.append(zone);setPlatform(platform);};window.reset=function(data){reset(data);};window.dispatch=dispatch;}());